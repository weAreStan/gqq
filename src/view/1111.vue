	...
	<div class="analyse-info-wrap">
		...
		<div class="analyse-info-top">
			...
			<div class="analyse-info-top-blacklist analyse-info-top-items">
				...
				<a-table class="analyse-info-top-table" :rowKey="(record) => record.suspectName"
					:columns="blackListColumns" :data-source="blackTableList" :pagination="false" :scroll="{ y: 400 }">
					<span slot="suspectName" style="color: #363637" slot-scope="text">
						<img src="../assets/logo.png" style="margin-right: 15px; height: 13.5px; width: 14px" />
						{{ text }}</span>
				</a-table>
			</div>
			<!-- 地图外层 -->
			...
			<div class="analyse-info-top-history analyse-info-top-items">
				...
				<a-table class="analyse-info-top-table" :columns="historyCollisionListColumns"
					:rowKey="(record) => record.suspectName" :data-source="historyCollisionTableList"
					...
					<span slot="content" style="color: #363637" slot-scope="text">
						...
						{{ text }}</span>
					<!-- 点击出现弹窗 -->
					<span slot="count" style="color: #178dfb; cursor: pointer" slot-scope="text, record"
		...
		<div class="analyse-info-bottom">
			<div class="analyse-info-bottom-suspects">
				...
				<div class="analyse-info-title-wrap analyse-info-title-suspects-wrap">
					<img src="../assets/logo.png" class="analyse-info-title-top-icon analyse-info-title-bottom-icon" />
					<h3 class="analyse-info-title">嫌疑人预测</h3>
				</div>
				<!-- 下方表单 -->
				<a-table class="analyse-info-top-table analyse-info-bottom-table"
					:rowKey="(record) => record.suspectName" :columns="suspectForecastListColumns"
					:data-source="suspectForecastTableList" :pagination="false" :scroll="{ y: 295 }">
		...
		<div class="histoy_modal">
			<a-modal v-model="visible" title="历史碰撞" @ok="handleOk" :width="'600px'" :bodyStyle="{ height: '500px' }">
				<!-- 第一部分 嫌疑主体 -->
				<div class="main_suspicion">
					<p class="main_name">
						嫌疑主体：<span class="blue_light" style="color: #1890ff; margin-left: 2px; cursor: pointer"
							@click="to_panorama">{{ dilogName }}</span>
					</p>
				</div>
				<!-- 表格 -->
				<div class="suspicion_case" style="margin-top: 20px">
					<a-table class="suspicion_case" :columns="columns_case" :data-source="data_case"
						:pagination="false">
						<span slot="case_supi" style="color: #1890ff; cursor: pointer" slot-scope="text"
				...
				<div class="footer" style="margin-top: 30px; margin-left: 10px">
					<a-pagination @showSizeChange="onShowSizeChange" size="small" :total="sizeTotal" show-size-changer
						show-quick-jumper @change="footer_change" :pageSizeOptions="pageSizeOptions"
