...
<template>
	<div class="full-view-detect-wrap">
		...
		<header class="full-view-detect-search-wrap">
			<div class="full-view-detect-search-to">
				<a-form layout="inline" :form="searchData">
					<a-form-item label="查询范围：">
						<a-select allowClear placeholder="请选择案件" @change="changeHandle($event, 'scope')"
							style="width: 200px">
							<a-select-option v-for="items in scopeList" :key="items.value" :value="items.value">
								{{ items.label }}
							</a-select-option>
						</a-select>
					</a-form-item>
					<a-form-item label="查询对象：">
						<a-select allowClear v-model="searchData.searchObj" style="width: 200px">
							<a-select-option v-for="items in searchObjList" :key="items.value" :value="items.value">
								{{ items.label }}
							</a-select-option>
						</a-select>
					</a-form-item>
					<a-form-item style="position: relative">
						<a-input placeholder="请输入" v-model.trim="searchData.searchObjContent"
							style="width: 320px; padding-right: 40px" />
						<span @click="toSearch" style="
                position: absolute;
                cursor: pointer;
                top: 50%;
			...
			<a-button @click="exportHandle" type="primary" class="full-view-detect-search-export">
				<i class="iconfont iconExportIocn" style="font-size: 14px; margin-right: 5px"></i>
				<!-- <a-icon type="export" /> -->
				导出
			</a-button>
		</header>
		...
		<section v-if="searchData.searchObj" class="full-view-detect-content-wrap">
			...
			<div class="full-view-detect-content-top">
				<div class="full-view-detect-content-top-detail-wrap">
					...
					<div class="full-view-detect-content-top-detail-left">
						...
						<ul class="full-view-detect-content-top-detail-left-tag" ref='tagContainer'>
							<li class="full-view-detect-content-top-detail-left-tag-items" ref='tags'
								v-for="(items, index) in tagList" :key="items.id" @mouseenter="enterTag(items, index)"
								@mouseleave="leaveTag(items)">
								<p :class="items.class">{{ items.tag }}</p>
								<p v-show="items.show" class="hoverClass" ref='popup'>
									<span v-for="(detail, index) in items.detailList" :key="index">{{ detail }}</span>
								</p>
							</li>
						</ul>
					</div>
					<!--  公司详情 -->
					<ul class="full-view-detect-content-top-detail-right" v-if="searchData.searchObj === 2">
					...
					<!--  个人 -->
					<ul class="full-view-detect-content-top-detail-right" v-if="searchData.searchObj === 1">
						<li class="full-view-detect-content-top-detail-right-items">
					...
					<img class="full-view-detect-content-top-detail-img" src="../assets/logo.png" alt="" width="100%"
					...
					<div class="swiper-container-wrap">
						...
						<div class="swiper-container" ref="swiper">
							<div class="swiper-wrapper">
								<div class="swiper-slide" v-for="(items, index) in 10" :key="index">
									<ul class="swiper-content">
										<li class="swiper-content-inner">
											<!-- 之前的src -->
											<!-- src="../../assets/imgs/proDbanklogo.png" -->
											<img style="width: 22px; height: 22px" src="../assets/logo.png" alt="" />
			...
			<div class="trisation_time">
				...
				<!-- 第二个柱状图 -->
				<div ref="second_histogram" style="width: 100%; height: 380px; background: #fff; margin-top: 30px">
				</div>
			</div>
			<!-- 交易金额特征 -->
			...
			<div class="trisation-map">
				...
				<div class="trisation-map-toggle">
					<p class="trisation-map-toggle-items trisation-map-toggle-people"
						@click="togglePeopleOrLocation(true)" :class="toggleData && 'active'">
						交易人员
					</p>
					<p class="trisation-map-toggle-items trisation-map-toggle-area"
						@click="togglePeopleOrLocation(false)" :class="!toggleData && 'active'">
						交易发生地
					</p>
				</div>
	...
	export default {
		...
		methods: {
			...
			async getPageInfo() {
				...
				// 获取银行卡信息
				let bankCardRes = await getBankCardList({
					type: 1,
					subject: 1
				});
