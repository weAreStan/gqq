	...
	<div class="user_wrapper">
		...
		<div class="search_add">
			<div class="search-wrapper">
				<a-input class="search-input" v-model="inputSearch" placeholder="请输入" @change.enter="searchLawName">
				</a-input>
		...
		<div class="table-wrapper">
			<a-table class="analyse-info-top-table" :columns="userColums" :data-source="userData" :pagination="false">
				<span slot="action" slot-scope="text, record">
					<span style="font-size: 14px; color: #1890ff; cursor: pointer"
						@click="add_new_user(record)">编辑&nbsp;&nbsp;&nbsp;&nbsp;</span>
					<span>&nbsp;&nbsp;&nbsp;&nbsp;</span>

					<!--  cofirm确认弹框 -->
					<span style="font-size: 14px; color: #1890ff; cursor: pointer"
						@click="deleteUser(record)">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;删除</span>
				</span>
			</a-table>
		</div>
		<!-- 分页 -->
		...
		<!--  弹窗 -->
		<a-modal v-model="visible" :title="reportType" @ok="handleOk" @cancel="handleCansel" :width="'600px'"
			:bodyStyle="{ height: '400px' }">
			<a-form-model ref="ruleForm" :model="form_wrapper" :rules="rules" :label-col="labelCol"
				:wrapper-col="wrapperCol">
				<a-form-model-item label="用户名：" ref="name" prop="name">
					<a-input :style="{ width: inputW, height: '32px' }" v-model.trim="form_wrapper.name"
						placeholder="请输入用户名" />
				</a-form-model-item>
				...
				<a-form-model-item label="用户密码：" style="position: relative;" has-feedback ref="user_pass"
					prop="user_pass">
					<a-input-password :disabled="forbidPass" placeholder="请输入密码" v-model.trim="form_wrapper.user_pass"
						:style="{ width: inputW }" />
					<a-button type="link" v-if="forbidPass" @click="forbidPass = false"
				...
				<a-form-model-item label="联系电话：" ref="phone" prop="phone">
					<a-input :style="{ width: inputW, height: '32px' }" v-model.trim="form_wrapper.phone"
						placeholder="请输入联系电话" />
				</a-form-model-item>
				<a-form-model-item label="邮箱：" ref="email" prop="email">
					<a-input :style="{ width: inputW, height: '32px' }" v-model.trim="form_wrapper.email"
						placeholder="请输入邮箱" />
				</a-form-model-item>
				<a-form-model-item label="备注：" ref="content" prop="content">
					<a-textarea v-model.trim="form_wrapper.content" style="resize: none" placeholder="请输入备注" rows="2">
					</a-textarea>
				</a-form-model-item>
			</a-form-model>
		</a-modal>
