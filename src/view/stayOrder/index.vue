	...
	<div class="stay-order">
		...
		<div class="operation">
			...
			<div class="wait_form_search">
				<a-input class="search-wrapper-input" v-model="inputSearch" placeholder="请输入户名/卡号"
					@change.enter="searchLawName" style="width: 323px; height: 32px">
					<i slot="suffix" class="iconfont iconSearchIcon" style="color: #1890ff"></i>
				</a-input>
			</div>
		...
		<div class="stay_table">
			<a-table :columns="columns" :data-source="wait_data" :scroll="{ x: 3000, y: 400 }" :pagination="false">
				<!--  户名 -->
				<a slot="name" slot-scope="name" style="cursor: pointer" @click="topanorama">
					{{ name }}
				</a>

				<span slot="accountNameTitle">户名</span>
				<!-- 卡号 -->
				<a slot="card_number" slot-scope="card_number" style="cursor: pointer" @click="topanorama">
					{{ card_number }}
				</a>
				...
				<span slot="placeTitle">
					...
					<a-cascader :options="options" :load-data="loadData" v-if="show_cascender_city" placeholder="请选择归属地"
						changeOnSelect="true" @change="onChange" @blur="hideSelect" :autoFocus="true" />
					<!-- 之前的src -->
				...
				<!--  总交易金额 -->
				<span slot="receipts_amount" slot-scope="receipts_amount" style="color: #1890ff">
					{{ receipts_amount }}
				</span>
				<span slot="allMoneyTitle">总交易金额</span>

				<!-- 进账金额 -->
				<span slot="trade_amount" slot-scope="trade_amount" style="color: #1890ff">
					{{ trade_amount }}
				</span>
				<span slot="enterMoneyTitle">进账金额</span>
				<!-- 出账金额 -->
				<span slot="expenditure_count" slot-scope="expenditure_count" style="color: #1890ff">
					{{ expenditure_count }}
				</span>
				<span slot="outMoneyTitle">出账金额</span>
				<span slot="action" slot-scope="text, record">
					<span style="font-size: 14px; color: #1890ff; cursor: pointer"
						@click="dialogShow(true, 'edit', record)">编辑&nbsp;&nbsp;&nbsp;&nbsp;</span>
					<span>&nbsp;&nbsp;&nbsp;&nbsp;</span>
					<!--  cofirm确认弹框 -->
					<span @click="deleteData(record)"
						style="font-size: 14px; color: #1890ff; cursor: pointer">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;删除</span>
				</span>
			</a-table>
			<div class="footer">
				<a-pagination @showSizeChange="onShowSizeChange" :page-size="params.pageNum"
					:default-current="params.pageIndex" size="small" :total="sizeTotal" show-size-changer
					show-quick-jumper class="pagination" @change="footer_change" :pageSizeOptions="pageSizeOptions"
					:defaultPageSize="defaultPageSize" :show-total="(total) => `共 ${total} 条`" />
			</div>
		...
		<!-- 弹窗 -->
		<Popup ref="popup" :visible="show" @dialogShow="dialogShow" @submit="submit" :title="title" />
	</div>
</template>
...
<script>
	...
	const columns = [{
		...
		},
		{
			...
			scopedSlots: {
				customRender: 'attribution_place'
			},
			width: 250,
			align: 'center',
			// filters: city,
		...
		},
		{
			...
			slots: {
				title: 'enterMoneyTitle'
			},
			scopedSlots: {
				customRender: 'trade_amount'
			},
			width: 150,
			align: 'center',
		},
		...
		},
		{
			...
			scopedSlots: {
				customRender: 'expenditure_count'
			},
			width: 150,
			align: 'center',
		},
